...
//        }
        ...
        floatingActionButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(linkedList!=null && linkedList.size()>0) {
                    cancelYelpCallBack();
                    message.setListYelp(linkedList);
                    Log.d(TAG,"Sending info: "+message.toString());
                    Intent intent = new Intent(SearchActivity.this, PersonalPlanActivity.class);
    ...
    private void populateList(String query, String sort,int pageAux) {
        ...
        if(networkUtil.connectionPermitted(this)){
            call = yelpAPI.search(postalCode, params);
    ...
    public void onItemDismiss(final int position, int direction) {
        if(direction==LEFT){
            Snackbar.make((CoordinatorLayout) findViewById(R.id.main_content), R.string.snackbar_delete_text, Snackbar.LENGTH_LONG)
                    .show();
        }
        ...
        this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                adapter.notifyItemRangeChanged(position, businesses.size()-position);
            }
        });
